...
class EnsembleMLTrainer:
    ...
    def train_global_classification_ensembles(self, global_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Stage 3: Diaphragm Type Classification
        if 'diaphragm_type' in global_df.columns:
            results['diaphragm'] = self._train_classification_ensemble(
                base_features, global_df['diaphragm_type'],
                self.diaphragm_encoder, 'Diaphragm Type'
            )
            self.diaphragm_ensemble = results['diaphragm']['model']
        
        # Stage 3: Plan Shape Classification
        if 'plan_shape' in global_df.columns:
            results['plan_shape'] = self._train_classification_ensemble(
                base_features, global_df['plan_shape'],
                self.plan_shape_encoder, 'Plan Shape'
            )
            self.plan_shape_ensemble = results['plan_shape']['model']
        ...
        # Stage 4: SFRS Classification
        if 'sfrs' in global_df.columns:
            results['sfrs'] = self._train_classification_ensemble(
                base_features, global_df['sfrs'],
                self.sfrs_encoder, 'SFRS'
            )
            self.sfrs_ensemble = results['sfrs']['model']
    ...
    def predict_global_properties(self, global_features: Dict[str, float]) -> Dict[str, Any]:
        ...
        # Predict diaphragm type
        if self.diaphragm_ensemble:
            diaphragm_pred = self.diaphragm_ensemble.predict(features_scaled)[0]
            diaphragm_probs = self.diaphragm_ensemble.predict_proba(features_scaled)[0]
            diaphragm_type = self.diaphragm_encoder.inverse_transform([diaphragm_pred])[0]
            results['DiaphragmType'] = diaphragm_type
            results['DiaphragmConfidence'] = max(diaphragm_probs)
        
        # Predict plan shape
        if self.plan_shape_ensemble:
            plan_pred = self.plan_shape_ensemble.predict(features_scaled)[0]
            plan_probs = self.plan_shape_ensemble.predict_proba(features_scaled)[0]
            plan_shape = self.plan_shape_encoder.inverse_transform([plan_pred])[0]
            results['PlanShape'] = plan_shape
            results['PlanShapeConfidence'] = max(plan_probs)
        ...
        # Predict SFRS
        if self.sfrs_ensemble:
            sfrs_pred = self.sfrs_ensemble.predict(features_scaled)[0]
            sfrs_probs = self.sfrs_ensemble.predict_proba(features_scaled)[0]
            sfrs = self.sfrs_encoder.inverse_transform([sfrs_pred])[0]
            results['SFRS'] = sfrs
            results['SFRSConfidence'] = max(sfrs_probs)
        ...
        special_features = []
        if global_features.get('has_cantilever', 0) > 0.5:
            special_features.append('Cantilever')
        if global_features.get('has_moment_frame', 0) > 0.5:
            special_features.append('Moment Frame')
        if global_features.get('has_braced_frame', 0) > 0.5:
            special_features.append('Braced Frame')
        if global_features.get('has_truss_system', 0) > 0.5:
            special_features.append('Truss System')
        
        results['SpecialFeatures'] = special_features
    ...
    def save_models(self):
        ...
        if self.diaphragm_ensemble:
            joblib.dump(self.diaphragm_ensemble, self.models_dir / 'diaphragm_ensemble.pkl')
            joblib.dump(self.diaphragm_encoder, self.models_dir / 'diaphragm_encoder.pkl')
        
        if self.plan_shape_ensemble:
            joblib.dump(self.plan_shape_ensemble, self.models_dir / 'plan_shape_ensemble.pkl')
            joblib.dump(self.plan_shape_encoder, self.models_dir / 'plan_shape_encoder.pkl')
        ...
        if self.sfrs_ensemble:
            joblib.dump(self.sfrs_ensemble, self.models_dir / 'sfrs_ensemble.pkl')
            joblib.dump(self.sfrs_encoder, self.models_dir / 'sfrs_encoder.pkl')
    ...
    def load_models(self) -> bool:
        ...
        try:
            ...
            if (self.models_dir / 'diaphragm_ensemble.pkl').exists():
                self.diaphragm_ensemble = joblib.load(self.models_dir / 'diaphragm_ensemble.pkl')
                self.diaphragm_encoder = joblib.load(self.models_dir / 'diaphragm_encoder.pkl')
            
            if (self.models_dir / 'plan_shape_ensemble.pkl').exists():
                self.plan_shape_ensemble = joblib.load(self.models_dir / 'plan_shape_ensemble.pkl')
                self.plan_shape_encoder = joblib.load(self.models_dir / 'plan_shape_encoder.pkl')
            ...
            if (self.models_dir / 'sfrs_ensemble.pkl').exists():
                self.sfrs_ensemble = joblib.load(self.models_dir / 'sfrs_ensemble.pkl')
                self.sfrs_encoder = joblib.load(self.models_dir / 'sfrs_encoder.pkl')
    ...
    def plot_feature_importance(self, model_type: str = 'member'):
        """Plot feature importance for specified model type"""
        if model_type == 'member' and self.member_ensemble:
            # Get feature importance from Random Forest component
            rf_model = self.member_ensemble.named_estimators_['rf']
            ...
            title = 'Member Role Classification Feature Importance'
        else:
            logger.warning(f"No feature importance data for {model_type}")
            return
...
def main():
    ...
    trainer.plot_feature_importance('member')
